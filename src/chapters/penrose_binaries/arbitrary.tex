...
\end{equation}
%
and the variables $X$ and $Y$ with $V^1$ and $V^2$, respectively. This quadratic describes a non-degenerate real ellipse in the $V^1$-$V^2$ plane if after defining
%
\begin{equation}
  \Delta = 
  \begin{vmatrix}
...
\end{equation}
%
one has $\Delta \neq 0$, $J > 0$, $\Delta/I < 0$ and $A \neq C$~\cite{Hart2002}. This ellipse is centered around the points $V^1_\circ$ and $V^2_\circ$, given by~\cite{Larson2006, Young2010, Lawrence2014}
%
\begin{align}
  V^1_\circ & = \frac{C D - B F}{B^2 - A C} \label{eq:arbitrary_penrose_ellipse_center_x} \\
  V^2_\circ & = \frac{A F - B D}{B^2 - A C} \label{eq:arbitrary_penrose_ellipse_center_y}
\end{align}